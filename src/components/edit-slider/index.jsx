        ...
        <div className="col-lg-12">
          <div className="d-flex justify-content-center my-3">
            <div>
              <div className="input-wrapper">
                <input type="file" name="file" id="input-file" className="input-file" onChange={onAddImage} />
          ...
          <div className="images-wrapper">
            {images.map((image, index) => {
              return (
                <div className="d-inline position-relative image-wrapper" key={index} >
                  <Image src={image} className="p-2 slider-image" />
                  <XLg className="delete-icon" onClick={() => deleteImage(index)} />
                </div>
              )
            })}
            {images.length !== 0 ? null : <h5 className="text-center mt-3">No images found</h5>}
          </div>
        </div>
      </div>
    </div>
